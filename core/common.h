...
static inline real K(int poreSize) { // Units: 1 / (nm^3*bar)
...
static inline real DSelf(int poreSize) {
...
static inline void createTables() {
    if(filled) return;
    for(int i=0; i<51; i++) {
        K_tabulated[i] = K(i);
...
static inline real fugacity(real concentration, int poreSize) {
    return concentration*K_inv_tabulated[poreSize];
}

static inline real concentration(real fugacity, int poreSize) {
